...
class CameraController extends PositionComponent {
  late final double speedModifier;

  final Vector2 limitMin;
  final Vector2 limitMax;
  ...
  CameraController(
      this.limitMin, this.limitMax, this.camera, this.speedModifier) {
    camera.speed = 100;
  }
  ...
  void onDragUpdate(DragUpdateInfo details) {
    ...
    currentSpeed = -details.delta.viewport * 2 * speedModifier;
  }
  ...
  void onDragEnd(DragEndInfo details) {
    ...
    currentSpeed = actualVelocity /
        previousMousePositions.length.toDouble() *
        speedModifier /
        3;
  }
}
